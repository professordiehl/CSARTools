    ...
    enrolmentsList.forEach(function (enrolment) {
        if (EXPORT_FORMAT === 'tsv') {
            dataRows.push([[
                    ...
                    enrolment.user.login_id + '@oakschristianuni.ac.nz',
...
function createSectionDataSpecifyHeader(section, withHeader) {
    ...
    section.students.forEach(function (student) {
        ...
        if (EXPORT_FORMAT === 'tsv') {
            dataRows.push([[section.name, student.name, student.sortable_name, student.sis_user_id, student.sis_login_id, student.sis_login_id + '@oakschristianuni.ac.nz'].join('\t')]);

        } else if (EXPORT_FORMAT === 'xlsx') {
            dataRows.push([section.name, student.name, student.sortable_name, student.sis_user_id, student.sis_login_id, student.sis_login_id + '@oakschristianuni.ac.nz']);
            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                if (members.length > 0) {
                    ...
                    members.forEach(function (member) {
                        if (EXPORT_FORMAT === 'tsv') {
                            dataRows.push([[group.name, member.name, member.sortable_name, member.sis_user_id, member.sis_login_id, member.sis_login_id + '@oakschristianuni.ac.nz'].join('\t')]);

                        } else if (EXPORT_FORMAT === 'xlsx') {
                            dataRows.push([group.name, member.name, member.sortable_name, member.sis_user_id, member.sis_login_id, member.sis_login_id + '@oakschristianuni.ac.nz']);
                        }
                    });
                }
